...
@login_required
def my_loans1(request):
    """View all loans for the current user"""
    loans = LoanProduct.objects.filter(customer=request.user).order_by('-created_at')
...
def my_loans(request):
    """
    View all loans for the current user.
    Shows DRAFT (deposit in progress) and ACTIVE loans.
    """
    # ✅ Get customer's loans - exclude completed/closed
    loans = LoanProduct.objects.filter(
        customer=request.user
    ).exclude(
        status__in=['PAID_OFF', 'CLOSED', 'CANCELLED', 'WRITTEN_OFF']
    ).order_by('-created_at')
    ...
    # ✅ Add helper flags to each loan (don't assign to properties)
    for loan in loans:
        # ✅ These are properties - just read them, don't assign
        # loan.deposit_target is a field - it's fine
        # loan.deposit_paid is a field - it's fine
        # loan.deposit_remaining is a PROPERTY - don't assign!
        # loan.deposit_percentage is a PROPERTY - don't assign!

        # ✅ Helper flags for template (these are NOT properties)
        loan.can_pay_deposit = not loan.deposit_complete and loan.status in ['DRAFT', 'ACTIVE']
        loan.can_pay_installment = loan.deposit_complete and loan.status == 'ACTIVE'
        loan.is_draft = loan.status == 'DRAFT'
        loan.is_active = loan.status == 'ACTIVE'
    ...
    context = {
        'loans': loans,
        'has_active_loans': any(loan.is_active for loan in loans),
        'has_draft_loans': any(loan.is_draft for loan in loans),
...
def my_payments(request):
    ...
    payments = PaymentRecord.objects.filter(customer=request.user).order_by('-created_at')
    return render(request, 'core/my_payments.html', {'payments': payments})
...
def make_payment(request):
    ...
    loans = LoanProduct.objects.filter(
        customer=request.user,
        status__in=['ACTIVE', 'DELINQUENT']
    ).order_by('-created_at')
    return render(request, 'core/make_payment.html', {'loans': loans})

@login_required
def profile(request):
    """User profile"""
    # Get counts
    loans_count = LoanProduct.objects.filter(customer=request.user).count()
    payments_count = PaymentRecord.objects.filter(customer=request.user).count()
